...
public class HebrewDate implements Comparable, Cloneable
{
	...
	/** Estas variables son para pasar la fecha en hebreo - Sami Pesate */
	public static String CURRENT_HMONTH = " ";
	public static int CURRENT_HDAY = 0;
	public static int CURRENT_HYEAR = 0;
	public static Boolean IS_LEAP_YEAR = false;

	private static final String[] hebrewMonths= {"Nissan", "Iyar",	"Sivan", "Tamuz", "Av", "Elul", "Tishrei", "Cheshvan", "Kislev", "Tevet", "Shvat", "Adar", "Adar II"};
	...
	* returns true if the year is an Hebrew leap year. */
	private static boolean isHebrewLeapYear(int year)
	{
	...
	* returns the last month of the Hebrew year. That is, either 12 or 13. */
	private int getLastMonthOfHebrewYear()
	{
		if (isHebrewLeapYear())
			return 13;
		else
	...
	* Returns last day of a hebrew month. */
	private int getLastDayOfHebrewMonth(int month)
	{
		if ((month == 2)
			|| (month == 4)
			|| (month == 6)
	...
	* Returns last day of hebrew month.  */
	...
	HebrewDate(int month, int date, int year) throws HebrewDateException
	{
	...
	* e.g. "Teves 23, 5760" */
	...
	{
		CURRENT_HDAY = hebrewDate;
		CURRENT_HMONTH = getHebrewMonthAsString();
		CURRENT_HYEAR = hebrewYear;
		IS_LEAP_YEAR = isHebrewLeapYear();
	...
	* equal. */
	...
	private String getHebrewMonthAsString()
	{
